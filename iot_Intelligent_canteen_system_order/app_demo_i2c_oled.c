...
hi_u8 ndefFileOrder[NDEF_DATA_LEN_MAX] = {
    //0x00, 0x28,
...
hi_u8 ndefFilePay[NDEF_DATA_LEN_MAX] = {
    //0x00, 0x28,
    //0xd4, 0x0f, 0x16, 0x61, 0x6e, 0x64, 0x72, 0x6f,
    //0x69, 0x64, 0x2e, 0x63, 0x6f, 0x6d, 0x3a, 0x70,
    //0x6b, 0x67, 0x63, 0x6f, 0x6d, 0x2e, 0x6e, 0x65,
...
static void OledNfcDisplayInit(void)
{
    ...
    OledShowStr(OLED_X_POSITION_0, OLED_Y_POSITION_0, \
                "Welcome LOFF P:0", OLED_DISPLAY_STRING_TYPE_1); /* 0, 0, xx, 1 */
    OledShowStr(OLED_X_POSITION_0, OLED_Y_POSITION_3, \
                "    NFC Mode    ", OLED_DISPLAY_STRING_TYPE_1); /* 15, 1, xx, 1 */
}

static void NFCTagOrderMode(void)
{
    OledShowStr(OLED_X_POSITION_0, OLED_Y_POSITION_6, \
                "     Order     ", OLED_DISPLAY_STRING_TYPE_1); /* 0, 2, xx, 1 */
    SetNdefData();
    int ret = GetNdefData(ndefFileOrder, sizeof(ndefFileOrder));
    if (ret != 0) {
        printf("get ndefFile Order Failed\r\n");
    }
}

static void NFCTagPayMode(void)
{
    OledShowStr(OLED_X_POSITION_0, OLED_Y_POSITION_6, \
                "      Pay      ", OLED_DISPLAY_STRING_TYPE_1); /* 0, 2, xx, 1 */
    SetNdefData();
    int ret = GetNdefData(ndefFilePay, sizeof(ndefFilePay));
    if (ret != 0) {
        printf("get ndefFile Pay Failed\r\n");
    }
}
